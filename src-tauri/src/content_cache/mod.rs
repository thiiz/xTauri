...
impl ContentCache {
    ...
    pub fn clear_profile_content(&self, profile_id: &str) -> Result<()> {
        ...
        tx.execute("DELETE FROM xtream_channels WHERE profile_id = ?1", [profile_id])?;
        tx.execute("DELETE FROM xtream_movies WHERE profile_id = ?1", [profile_id])?;
        tx.execute("DELETE FROM xtream_series WHERE profile_id = ?1", [profile_id])?;
        tx.execute("DELETE FROM xtream_seasons WHERE profile_id = ?1", [profile_id])?;
        tx.execute("DELETE FROM xtream_episodes WHERE profile_id = ?1", [profile_id])?;
        tx.execute("DELETE FROM xtream_channel_categories WHERE profile_id = ?1", [profile_id])?;
        tx.execute("DELETE FROM xtream_movie_categories WHERE profile_id = ?1", [profile_id])?;
        tx.execute("DELETE FROM xtream_series_categories WHERE profile_id = ?1", [profile_id])?;
    ...
    pub fn get_content_counts(&self, profile_id: &str) -> Result<(usize, usize, usize)> {
        ...
        Ok((channels_count as usize, movies_count as usize, series_count as usize))
    }
    ...
    pub fn get_channels(&self, profile_id: &str, filter: Option<ChannelFilter>) -> Result<Vec<XtreamChannel>> {
        ...
        let channels = stmt.query_map(param_refs.as_slice(), |row| {
            Ok(XtreamChannel {
                stream_id: row.get(0)?,
                num: row.get(1)?,
                name: row.get(2)?,
    ...
    pub fn save_series_details(&self, profile_id: &str, series_id: i64, details: XtreamSeriesDetails) -> Result<()> {
        validate_profile_id(profile_id)?;
    ...
    pub fn get_series(&self, profile_id: &str, filter: Option<SeriesFilter>) -> Result<Vec<XtreamSeries>> {
        validate_profile_id(profile_id)?;
        ...
        let series = stmt.query_map(param_refs.as_slice(), |row| {
            Ok(XtreamSeries {
                series_id: row.get(0)?,
                num: row.get(1)?,
                name: row.get(2)?,
    ...
    /// # Returns
    /// Complete series details with seasons and episodes
    pub fn get_series_details(&self, profile_id: &str, series_id: i64) -> Result<XtreamSeriesDetails> {
        validate_profile_id(profile_id)?;
        validate_stream_id(series_id)?;
        ...
        let series = conn.query_row(
            ...
            rusqlite::Error::QueryReturnedNoRows => {
                XTauriError::content_cache(format!("Series {} not found for profile {}", series_id, profile_id))
            }
        ...
        let seasons = stmt.query_map(params![profile_id, series_id], |row| {
            Ok(XtreamSeason {
        ...
        let episodes = stmt.query_map(params![profile_id, series_id], |row| {
            Ok(XtreamEpisode {
    ...
    /// # Returns
    /// Vector of episodes
    pub fn get_episodes(&self, profile_id: &str, series_id: i64, season_number: Option<i64>) -> Result<Vec<XtreamEpisode>> {
        validate_profile_id(profile_id)?;
        validate_stream_id(series_id)?;
        ...
        let (query, params): (String, Vec<Box<dyn rusqlite::ToSql>>) = if let Some(season) = season_number {
            (
                "SELECT episode_id, season_number, episode_num, title,
                        container_extension, custom_sid, added, direct_source, info_json
                 FROM xtream_episodes
                 WHERE profile_id = ?1 AND series_id = ?2 AND season_number = ?3
                 ORDER BY CAST(episode_num AS INTEGER)".to_string(),
                vec![
                    Box::new(profile_id.to_string()),
                    Box::new(series_id),
                    Box::new(season),
            ...
            (
                "SELECT episode_id, season_number, episode_num, title,
                 ...
                 FROM xtream_episodes
                 WHERE profile_id = ?1 AND series_id = ?2
                 ORDER BY season_number, CAST(episode_num AS INTEGER)".to_string(),
                vec![
                    Box::new(profile_id.to_string()),
                    Box::new(series_id),
                ],
        ...
        let episodes = stmt.query_map(param_refs.as_slice(), |row| {
            Ok(XtreamEpisode {
                episode_id: row.get(0)?,
                season_number: row.get(1)?,
                episode_num: row.get(2)?,
    ...
    pub fn fts_search_channels(&self, profile_id: &str, query: &str, filter: Option<ChannelFilter>) -> Result<Vec<XtreamChannel>> {
        validate_profile_id(profile_id)?;
    ...
    pub fn fts_search_movies(&self, profile_id: &str, query: &str, filter: Option<MovieFilter>) -> Result<Vec<XtreamMovie>> {
        validate_profile_id(profile_id)?;
    ...
    pub fn fts_search_series(&self, profile_id: &str, query: &str, filter: Option<SeriesFilter>) -> Result<Vec<XtreamSeries>> {
        validate_profile_id(profile_id)?;
    ...
    pub fn save_categories(
        ...
        let saved = batch_insert(&mut conn, table_name, &categories, |tx, category| {
            ...
            if category.category_id.trim().is_empty() {
                return Err(XTauriError::profile_validation("category_id cannot be empty"));
            }
    ...
    pub fn get_categories(
        ...
        let categories = stmt.query_map(param_refs.as_slice(), |row| {
            Ok(XtreamCategory {
    ...
    pub fn get_content_ids(&self, profile_id: &str, content_type: &str) -> Result<Vec<i64>> {
        ...
        let mut stmt = conn.prepare(&query)?;
        let ids = stmt.query_map([profile_id], |row| row.get::<_, i64>(0))?
            .collect::<std::result::Result<Vec<_>, _>>()?;
