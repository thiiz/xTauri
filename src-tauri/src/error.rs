...
impl XTauriError {
    ...
    pub fn file_download(url: impl Into<String>) -> Self {
        Self::FileDownload { url: url.into() }
    }
    ...
    pub fn xtream_profile_not_found(id: impl Into<String>) -> Self {
        Self::XtreamProfileNotFound { id: id.into() }
    }
    ...
    pub fn is_recoverable(&self) -> bool {
        match self {
            ...
            XTauriError::Cache { .. } | XTauriError::SearchCache { .. } => true,
            ...
            XTauriError::XtreamAuthenticationFailed { .. } | XTauriError::XtreamApiError { .. } => {
                true
            }
    ...
    pub fn user_message(&self) -> String {
        match self {
            ...
            XTauriError::PlaylistFetch { .. } => {
                "Failed to load playlist. Please check the URL and try again.".to_string()
            }
            XTauriError::FileDownload { .. } => {
                "Failed to download file. Please check your connection and try again.".to_string()
            }
            XTauriError::DataDirectoryAccess => {
                "Cannot access application data directory. Please check permissions.".to_string()
            }
            XTauriError::M3uParsing { .. } => {
                "Invalid playlist format. Please check the playlist file.".to_string()
            }
            XTauriError::InvalidUrl { .. } => {
                "Invalid URL format. Please check the URL and try again.".to_string()
            }
            ...
            XTauriError::XtreamAuthenticationFailed { .. } => {
                "Failed to authenticate with Xtream server. Please check your credentials."
                    .to_string()
            }
            XTauriError::XtreamInvalidCredentials => {
                "Invalid Xtream credentials. Please check your username, password, and server URL."
                    .to_string()
            }
            XTauriError::XtreamProfileNotFound { .. } => "Xtream profile not found.".to_string(),
            XTauriError::XtreamApiError { .. } => {
                "Xtream server error. Please try again later.".to_string()
            }
            XTauriError::CredentialEncryption { .. } | XTauriError::CredentialDecryption { .. } => {
                "Failed to process credentials securely.".to_string()
            }
            XTauriError::ContentCache { .. } => {
                "Content cache error. Data will be refreshed.".to_string()
            }
    ...
    pub fn category(&self) -> &'static str {
        match self {
            ...
            XTauriError::FileSystem(_)
            | XTauriError::DirectoryCreation { .. }
            | XTauriError::DataDirectoryAccess
            | XTauriError::FileRead { .. }
            | XTauriError::FileWrite { .. } => "filesystem",
            XTauriError::M3uParsing { .. }
            | XTauriError::UrlParsing { .. }
            | XTauriError::RegexError { .. } => "parsing",
            XTauriError::Cache { .. } | XTauriError::SearchCache { .. } => "cache",
            XTauriError::Configuration { .. } | XTauriError::InvalidSetting { .. } => {
                "configuration"
            }
            ...
            | XTauriError::FeatureNotAvailable { .. }
            | XTauriError::NotFound { .. } => "state",
            XTauriError::XtreamAuthenticationFailed { .. }
            | XTauriError::XtreamInvalidCredentials
            | XTauriError::XtreamProfileNotFound { .. }
            | XTauriError::XtreamApiError { .. } => "xtream",
            XTauriError::CredentialEncryption { .. } | XTauriError::CredentialDecryption { .. } => {
                "security"
            }
...
mod tests {
    ...
    fn test_error_categories() {
        assert_eq!(
            XTauriError::Database(rusqlite::Error::InvalidPath("test".into())).category(),
            "database"
        );
        assert_eq!(
            XTauriError::PlaylistFetch {
    ...
    fn test_recoverable_errors() {
        assert!(XTauriError::PlaylistFetch {
            url: "test".to_string()
        }
        .is_recoverable());
        assert!(XTauriError::Timeout {
            operation: "test".to_string()
        }
        .is_recoverable());
        assert!(XTauriError::Cache {
            operation: "test".to_string()
        }
        .is_recoverable());
        assert!(XTauriError::LockAcquisition {
            resource: "test".to_string()
        }
        .is_recoverable());

        assert!(!XTauriError::DatabaseInitialization {
            reason: "test".to_string()
        }
        .is_recoverable());
    ...
    fn test_user_messages() {
        ...
        let network_error = XTauriError::PlaylistFetch {
