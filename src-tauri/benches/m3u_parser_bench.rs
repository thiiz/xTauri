...
fn generate_complex_m3u_content(channel_count: usize) -> String {
    ...
    for i in 0..channel_count {
        ...
        let url = match i % 3 {
            0 => format!(
                "http://stream{}.example.com/live/channel{}/playlist.m3u8",
                i % 10,
                i
            ),
            1 => format!(
                "https://live.example.com/hls/ch{}/index.m3u8?token=abc123",
                i
            ),
...
fn bench_m3u_parsing_with_progress(c: &mut Criterion) {
    ...
    group.bench_function("with_progress_callback", |b| {
        b.iter(|| {
            ...
            let channels =
                parse_m3u_content_with_progress(black_box(&content), black_box(progress_callback));
            black_box(channels)
        });
    });
